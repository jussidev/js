...
function fetchApi(url){
    ...
    .then((data) => {
        let tempInCelcius = ((5/9) * (data.main.temp-32)).toFixed(1);

        city.textContent      = data.name
        temperatura.innerHTML = tempInCelcius
        humidity.innerHTML    = data.main.humidity
    })

    .catch((err) => {
        city.innerHTML = `Impossível acessar o OpenWeather. Verificar a sua conexão.`;
